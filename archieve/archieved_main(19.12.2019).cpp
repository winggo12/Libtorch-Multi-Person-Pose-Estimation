...
int main(int argc, const char* argv[])
{
  ...
  //assert(module != nullptr);
    ...
    else
    {
        ...
        auto cropimagestart = std::chrono::high_resolution_clock::now();
        ...
        {   xmin[i] = result_data[i][1];
            ...
            //std::cout << xmin[i] << " " << ymin[i] << " " << xmax[i] << " " << ymax[i] << " " << std::endl;
            ...
            auto cropimageend = std::chrono::high_resolution_clock::now();
            auto cropimageduration = duration_cast<milliseconds>(cropimageend - cropimagestart); 
            std::cout << "Time taken for cropimage: " << cropimageduration.count() << " ms" << endl;


            // The difference btw .forward({x})<--x is torch::Tensor and .forward(x)<--x is std::vector<torch::jit::IValue>
            //int dim = 0;
            //auto input_ = torch::cat(input_vec , dim);
            //input_tensor.push_back(input_);
            input_vec = input_vec.to(at::kCUDA);
            auto sppestart = std::chrono::high_resolution_clock::now();
            auto output = sppemodule.forward({input_vec}); 
            torch::Tensor output_tensor = output.toTensor();
            output_tensor = output_tensor.to(at::kCPU);
            auto sppeend = std::chrono::high_resolution_clock::now();
            auto sppeduration = duration_cast<milliseconds>(sppeend - sppestart); 
            std::cout << "Time taken for sppeforward: " << sppeduration.count() << " ms" << endl;
            

            auto outputstart = std::chrono::high_resolution_clock::now();
             auto ft = output_tensor.flatten(2,3); //flattening
            auto maxresult = at::max(ft,2);
            auto max_index = std::get<1>(maxresult); 
            
            auto outputend = std::chrono::high_resolution_clock::now();
            auto outputduration = duration_cast<milliseconds>(outputend - outputstart); 
            std::cout << "Time taken for flattening: " << outputduration.count() << " ms" << endl;
            
            std::cout << "max result: " << max_index[0][1] << endl;
            std::cout << "output : " << max_index  << endl;

        for (int j = 0; j < result.size(0) ; j++){
            sppe.OutputtoOrigImg2(j,origin_image,max_index,width[j],height[j],xmin[j],ymin[j]);   
        }
